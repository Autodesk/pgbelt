...
You can also set `exclude_users` and `exclude_patterns` in config.json to define
default exclusions per migration. CLI flags are additive to config values.
...
- `-e, --exclude-user TEXT`: Additional usernames to exclude from revocation (can be repeated).
