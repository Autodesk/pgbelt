...
func renderConnections(connections []Connection) {
	...
	if IsBeltIntegrated {
		connTable.SetOutputMirror(&returnStr)
	} else {
...
func printSummary(resource string, srcCount, dstCount int, equals bool, missing int) {
	...
	fmt.Fprintf(outWriter, "Source Count: %s%d%s\n", green, srcCount, reset)
	fmt.Fprintf(outWriter, "%s Count: %s%d%s\n", resource, green, dstCount, reset)
	if equals {
		fmt.Fprintf(outWriter, "%s Count Equals: %s%v%s\n", resource, green, equals, reset)
	} else {
		fmt.Fprintf(outWriter, "%s Count Equals: %s%v%s\n", resource, red, equals, reset)
	}
	if missing > 0 {
		fmt.Fprintf(outWriter, "Missing %s Count: %s%d%s\n", resource, yellow, missing, reset)
	} else {
...
func CompareCommand(fileLocation string) string {
	t := table.NewWriter()
	i := table.NewWriter()
	s := table.NewWriter()
	seq := table.NewWriter()
	...
	if IsBeltIntegrated {
		t.SetOutputMirror(&returnStr)
		i.SetOutputMirror(&returnStr)
		s.SetOutputMirror(&returnStr)
		seq.SetOutputMirror(&returnStr)
	} else {
